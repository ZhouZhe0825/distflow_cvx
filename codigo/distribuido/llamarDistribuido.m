function [Var_dist_conE, Var_centr, Var_F, opt_dist_conE, opt_centr, opt_F, status, Data_d, Ev] = llamarDistribuido(Data, Config, util, Var_centr, opt_centr, Var_ini, opt_ini)
    ...
    if isempty(Var_centr) && isempty(Var_ini)
        ...
        [Var_centr, opt_centr, status] = distflowCentralizadoNxN(DataM, Config, true, util);
        ...
        [Var_ini, opt_ini, status] = distflowCentralizadoNxN(DataM, ConfigIni, true, util);
        if isNotSolved(status)
        	save(Config.workspace_var_file);
            return;
        end
...
%         DataM_f.Fixed.z = Var_ini.Red.Branch.z;
        ...
        [Var_ini_, opt_ini, status] = distflowCentralizadoNxN(DataM_f, ConfigIni, true, util);
        ...
        Var_ini.Gen.Dfig.pWi = Var_ini_.Gen.Dfig.pWi;
        Var_ini.Gen.Dfig.qWi = Var_ini_.Gen.Dfig.qWi;
    ...
    [DistrInfo] = actualizarDistrInfo(Var_ini, DataM, DfigNodes, ClNINodes, PvNodes, AlmNodes, DistrInfo);
    ...
    DistrInfo.mu = Var_ini.Dual.dPn;
    DistrInfo.lambda = Var_ini.Dual.dQn;
...
function [DistrInfo] = actualizarDistrInfo(Var, Data, DfigNodes, ClNINodes, PvNodes, AlmNodes, DistrInfo)
	...
	DistrInfo.Tras.P = Var.Red.Branch.P;
	DistrInfo.Tras.Q = Var.Red.Branch.Q;
...
function [Var_curr, opt_curr, status, it, DistrInfo, Ev, wbSol] = DistrLoop(ini_it, Fixed, DistrInfo, Config, Data, Ev, wbSol)
    ...
    while ~conv && it <= Config.Distr.TopIT
		...
		[DistrInfo] = actualizarDistrInfo(Var_curr, Data, DfigNodes, ClNINodes, PvNodes, AlmNodes, DistrInfo);
...
function [Var, opt, opt_Tras, opt_Dfig, opt_Pv, opt_ClNI, opt_ClRes, opt_Alm, opt_OpSis, status] = solve_all_subproblems(Data, Config, DistrInfo, it, wbSol, opt_Tras, opt_Dfig, opt_Pv, opt_ClNI, opt_ClRes, opt_Alm, opt_OpSis, opt, Fixed)
	...
	% Trasmision
	previo = 0;
	[Var, opt_Tras, opt, status_Tras] = solve_problem('Tras', previo, totalProb, opt_Tras, opt, @df_Tras, it, Data.Red.Bus.v0, true, wbSol, [], Data, Config, DistrInfo, Fixed);

	% Dfig
	previo = previo+1;
	[Var, opt_Dfig, opt, status_Dfig] = solve_problem('Dfig', previo, totalProb, opt_Dfig, opt, @df_Dfig, it, DfigNodes, false, wbSol, Var, Data, Config, DistrInfo, Fixed);

	% Pv
	previo = previo+length(DfigNodes);
	[Var, opt_Pv, opt, status_Pv] = solve_problem('Pv', previo, totalProb, opt_Pv, opt, @df_Pv, it, PvNodes, false, wbSol, Var, Data, Config, DistrInfo, Fixed);

	% ClNI
	previo = previo+length(PvNodes);
	[Var, opt_ClNI, opt, status_ClNI] = solve_problem('ClNI', previo, totalProb, opt_ClNI, opt, @df_ClNI, it, ClNINodes, false, wbSol, Var, Data, Config, DistrInfo, Fixed);

	% Clientes residenciales
	previo = previo+length(ClNINodes);
	[Var, opt_ClRes, opt, status_ClRes] = solve_problem('ClRes', previo, totalProb, opt_ClRes, opt, @df_ClRes, it, ClResNodes, true, wbSol, Var, Data, Config, DistrInfo, Fixed);

	% Almacenamiento de baterias
	previo = previo+length(ClResNodes);
	[Var, opt_Alm, opt, status_Alm] = solve_problem('Alm', previo, totalProb, opt_Alm, opt, @df_Alm, it, AlmNodes, false, wbSol, Var, Data, Config, DistrInfo, Fixed);

	% Red
	previo = previo+length(AlmNodes);
	[Var, opt_OpSis, opt, status_OpSis] = solve_problem('OpSis', previo, totalProb, opt_OpSis, opt, @df_OpSis, it, OpSisNodes, true, wbSol, Var, Data, Config, DistrInfo, Fixed);
...
% 	waitbar(previo/(totalProb),wbSol,sprintf(['Calculo de solucion distr - it ', num2str(it), ' - ', strProb]))
    	[Var_Prob_i, opt_Aux, status_Prob_i] = problem(Data_d, Config, true, DistrInfo);
